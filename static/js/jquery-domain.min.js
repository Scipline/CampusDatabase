...
(function($) {
    ...
    function collect(target) {
        ...
        t.find(":input[name]").not(":button,:reset,:image,:submit").each(function(ndx, ele) {
            ...
            var nodeName = box.get(0).nodeName;
            if(nodeName=='TEXTAREA'){
            	var reg1 = new RegExp("<","gi");
            	val = val.replace(reg1,'&#60');
            	var reg2 = new RegExp(">","gi");
            	val = val.replace(reg2,'&#62');
            }
            	...
            	if(box.is(":checked")){
            		if (typeof(collection[key]) == "undefined") {
                    	...
                    	if($.isArray(collection[key])) {
                    		collection[key].push(encodeURIComponent(val));
                    	} else {
                    		collection[key] = [collection[key], encodeURIComponent(val)];
                    	}
                    }
            	}
            	return true;
    ...
    function openDialog(target, options) {
        ...
        var dlg = target.$("<div style='overflow:hidden'><iframe scrolling='no' frameborder='0' style='width:100%;height:100%;' src='about:blank'></iframe></div>").appendTo(target.document.body);
    ...
    function load(target, options) {
    	...
    	build(target);
    	var t = $(target);
    	if(options && options.url && new RegExp("/(-?[a-zA-Z\\d]+)").test(options.url)) {
    ...
    function exist(target, options) {
        ...
        boxes.each(function(ndx, ele) {
            ...
            box.each(function(i, d) {
            	var b = $(this);
            	var v = $.trim(b.val());
            	vals.push(encodeURIComponent(v));
            })
    ...
    function create(target, options) {
        ...
        $.ajax({
            ...
            success: function(data, statusText, XHR) {
            	...
            	var m = $.format("添加 <font style='color:red'>{0}</font> {1}成功。", (data.code || data.name || ""), opts.title);
    ...
    function edit(target, options) {
        ...
        $.ajax({
            ...
            success: function(data, statusText, XHR) {
            	...
            	if(flag){
            		oraError(data);
            		opts.onLoadSuccess.call(target, data, statusText, XHR);
            	}else{
            		data = $.decodeURIComponent(opts.data);
            		data = formatData(target, data);
            		var m = $.format("编辑 <font style='color:red'>{0}</font> {1}成功。", (data.code || data.name || ""), opts.title);
            		top.$.messager.show({ title: "信息", msg: m, timeout: 4000, showType: "slide" });
    	...
    	top.$.messager.confirm("确认", m, function(result) {
            if (result) {
                $.ajax({
                    ...
                    success: function(data, status, XHR) {
                    	m = $.format("删除 <font style='color:red'>{0}</font> {1}及其下{2}成功。", text, title, title);
                        top.$.messager.show({ title: "信息", msg: m, timeout: 4000, showType: "slide" });
                ...
                $.ajax({
                    ...
                    success: function(data, status, XHR) {
                    	...
                    	if(flag){
                    		...
                    		m = $.format("成功删除 <font style='color:red'>{0}</font> 条{1}。", ids.length, title);
                			top.$.messager.show({ title: "信息", msg:m, timeout: 4000, showType: "slide" });
                			window.setTimeout(function() {
    ...
    function changeButtonState(target) {
    	...
    	var options = t.datagrid("options");
    	for(var i=0;i<length;i++){
    		var data = t.datagrid("getSelections")[i];
    		if(options.isFlow){
    			if(data.status!=null&&data.status!="undefined"&&(data.shlc!=null||data.flowId!=null||data.flowid!=null)){
    	...
    	for(var i = 0; i < btns.length; i++) {
    		...
    		else if(typeof(scope) == "string" &&edit=="") {
    			b.linkbutton("disable");
    			btn.disabled = true;
    			if(length == 0) {
    ...
    function nameCombo(target, options) {
    	...
    	var idField = "value";//getUrlParameterValue("idField", opts.url);
    	var nameField = "text";//getUrlParameterValue("nameField", opts.url);
        ...
        datagrid: $.extend({}, $.fn.datagrid.defaults, {
          ...
          //  pageSize: 15,
            ...
            toolbar_excel: [ "-", {
                ...
                disabled: !0,
                handler: function() {
                    var e = $("form.query").domain("collect"), t = "";
                    for (o in e) e[o] && e[o] != "" && (t = t + "&" + o + "=" + e[o]);
                    e || (e = {});
                    var n = $(this).parent().next().find(">table:hidden"), r = n.datagrid("options");
                    e.subject = r.subject;
                    var i = r.columns[0], s = 0;
                    for (var o = 0; o < i.length; o++) i[o] && (i[o].hidden == 0 || i[o].hidden == "") && (e["field_" + s] = i[o].field, e["title_" + s] = i[o].title, s++);
                    var u = r.url.split("/")[0] + "/" + r.url.split("/")[1];
                    $.ajax({
                        url: u + "/management/exportcontroller/setexportflag",
                        async: !1,
                        data: e,
                        success: function(e, n, i) {
                            var s = r.excelurl;
                            s = s + "?exportFlag=" + e + t, window.location.href = s;
                        }
                    });
                },
                scope: "all"
    ...
    $.fn.domain.create={
        ...
        handler: function() {
            ...
            var opts = t.datagrid("options");
            var title = opts.title || opts.subject;
            var url = opts.urlCreate || (opts.url.match(new RegExp("page", "i")) ? opts.url.replace(new RegExp("page", "i"), "edit/0") : undefined);
            if (!url) {
            	return;
            }
            $(parent).domain("openDialog", { 
...
function indexQuery(options){
	var objForm = options.objForm;
 	if (objForm.form('validate') == false) {
 		return false;
 	}
 	 var queryParams = options.queryParams;
     var objTable = options.objTable;
     
    if(typeof($.data(objTable[0], 'datagrid')) == 'undefined') {
    	var toolbar = options.toolbar;
    	
    	objTable.domain('datagrid', {
        	url: options.url,
            ...
            onLoadSuccess: function(data, status, XHR) {
                var box = objForm.find(':input[name][value!=""]:visible:last');
                if(box.length == 0) {
                    box = objForm.find(':input[name]:visible:first');
                }
                box.focus().select();
            },
            onLoadError: function(XHR, status, errorThrow) {
            ...
            openWidth: options.openWidth,
     		...
     		idField:options.idField==""?"id":options.idField
        });
    }
    else {
    	objTable.datagrid("load", queryParams);
    }
}

	

	function formatColumn(value, column) {
		var status = [{"value":-1,"text":"<span class='label label-success'>草稿</span>"},{"value":0,"text":"<span class='label label-warning'>已提交</span>"}
		,{"value":1,"text":"<span class='label label-success'>处理中</span>"},{"value":2,"text":"<span class='label label-warning'>完毕</span>"}
		,{"value":3,"text":"<span class='label label-success'>退回</span>"},{"value":4,"text":"<span class='label label-warning'>撤销</span>"}];
	    var items = column||status;
	...
	function chosenSelfCombox(){
		$(".chzn-select").each(function(index) {
			var obj = $(this);
			var selfValue = obj.attr("selfValue");
			...
			if(url.indexOf("?")>0){
			 ...
			 $.ajax({
		            ...
		            async:false,
		            url:url +"?date="+new Date().getTime(),//要访问的后台地址
		            success: function(msg){//msg为返回的数据，在这里做数据绑定
		            		var options = '<option value="-1">请选择...</option>';
		            		for(var i=0;i<msg.length;i++){
		            			if(selfValue==msg[i].value){
		            				options+='<option flowId="'+msg[i].extraField1+'" value='+msg[i].value+" selected='selected'>"+msg[i].text+"</option>";
		            			}else{
		            				options+='<option flowId="'+msg[i].extraField1+'" value='+msg[i].value+">"+msg[i].text+"</option>";
		            			}
		            		}
		            		obj.html(options);	      		
		            		obj.chosen();
	...
	function chosenFlowCombox(data){
		$(".chzn-select").each(function(index) {
			var obj = $(this);
			var url = $(this).attr("url");
			if(url==null){
				$(this).chosen();
				 ...
				 $.ajax({
			            ...
			            success: function(msg){//msg为返回的数据，在这里做数据绑定
			            		var options = '';
			            		for(var i=0;i<msg.length;i++){
			            			if(msg[i].extraField1){
			            				if(data!=null && data[obj.attr("name")]==msg[i].value){
			            					options+='<option flowId="'+msg[i].extraField1+'" value='+msg[i].value+" selected='selected'>"+msg[i].text+"</option>";
			            				}else{
			            					options+='<option flowId="'+msg[i].extraField1+'" value='+msg[i].value+">"+msg[i].text+"</option>";
			            				}
			            				...
			            				if(data!=null && data[obj.attr("name")]==msg[i].value){
			            					options+='<option value='+msg[i].value+" selected='selected'>"+msg[i].text+"</option>";
			            				}else{
		...
		$(".chzn-select").each(function(index) {
			...
			if($(this).attr("url")==null){
			 ...
			 $.ajax({
		            ...
		            dataType: "json",//返回json格式的数据
		            async:false,
		            url: $(this).attr("url")+"?date="+new Date().getTime(),//要访问的后台地址
		            success: function(msg){//msg为返回的数据，在这里做数据绑定
		            		var options = '<option value="-1">请选择...</option>';
		            		var b = obj.attr("name");
		            		for(var i=0;i<msg.length;i++){
	...
	function chosenDictionary(data){
		$(".chzn-select").each(function(index) {
			var obj = $(this);
			if($(this).attr("url")==null){
				$(this).chosen();
	...
	function chosenMultipleCombox(data){
		$(".chzn-select").each(function(index) {
			var obj = $(this);
			if($(this).attr("url")==null){
				$(this).chosen();
			 ...
			 $.ajax({
		            ...
		            success: function(msg){//msg为返回的数据，在这里做数据绑定
		            		...
		            		var b = obj.attr("name");
	            			for(var i=0;i<msg.length;i++){
		            			if(data!=null&&data!='' && data[obj.attr("name")].indexOf(msg[i].value)>-1){
		            				options+='<option value="'+msg[i].value+'" selected="selected" >'+msg[i].text+"</option>";
		            			}else{
		            				options+='<option value="'+msg[i].value+'">'+msg[i].text+"</option>";
		            			}
	...
	/**
	 * 数据库操作失败提示
	 * @param data oracle错误代码相反数
	 */
	function oraError(data){
		var msg;
		switch(data){
			case -2292:
				msg = "数据已经被使用！";
