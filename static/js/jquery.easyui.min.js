jQuery.extend({
    ...
    function _461(_462) {
        ...
        function _467(_46c, _46d, _46e) {
            ...
            $(_46c).show();
            $(_46c).empty();
            var t = $('<div class="table-responsive"><table class="table table-striped table-bordered table-hover" border="0" cellspacing="0" cellpadding="0"><thead></thead><tbody></tbody></table></div>').appendTo(_46c);
            for (var i = 0; i < _46d.length; i++) {
                var tr = '';
                if(i==0){
                	tr = $('<tr class="center"></tr>').appendTo($("thead", t));
                }else{
                	tr = $('<tr class="center"></tr>').appendTo($("tbody", t));
                }
                var cols = _46d[i];
    ...
    $.extend($.fn.validatebox.defaults.rules, {
    	emailNS: {
            validator: function(b,c) {
            	...
            	var d = /^.+cpu\.edu\.cn/g;
            	if(b){
            		 var flag = $.fn.validatebox.defaults.rules.length.validator(b, c) && a.test(b);
            		 if(flag){
            			 return !d.test(b);
            		 }else{
            			 return false;
            ...
            message: "请输入有效的邮箱，且必须是校外邮箱，且长度必须在{0} 和 {1} 之间"
        },
        integer: {
            validator: function(b, c) {
                var a = /^-?\d+$/;
